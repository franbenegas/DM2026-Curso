# Pagina Web de Datos, Ecuaciones Diferenciales e Inteligencia Artificial

Instructor: [Facundo Sapienza](https://facusapienza.org)



## Contribución a los apuntes de la materia

Las notas de la materia fueron creadas en colaboración con los estudiantes.
Como requisito para aprobar la materia, cada estudiante debe abrir un Pull Request con los apuntes de la clase para una dada fecha.
De esta manera, completamos los apuntes de las clases juntos y al mismo tiempo se pone en práctica la modalidad de trabajo de codigo abierto en un repositorio.

A continuación, una lista de los estudiantes que contribuyeron a cada clase.

| Clase         | Alumnos/as |
| ------------- | ------------- |
| No1           | ...  |
| No2           | Ali Martín Zynda ([@alimartinza](https://github.com/alimartinza)), Cecilia Bolaños ([@ceciliabolanos](https://github.com/ceciliabolanos)), Manuel Matías Drescher ([@ManuDrescher](https://github.com/ManuDrescher))|
| No3           | ...  |
| No4           | Francisco Castillo Menegotto ([@FrCastillo](https://github.com/FrCastilloM)), Lucas Grasso Ramos ([@LucasGrasso](https://github.com/LucasGrasso))  |
